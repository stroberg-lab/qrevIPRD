...
def calc_rdf(traj,nL,nbins,rrange,timerange=None):
    ...
    # Calculate RDF for each time in timerange
    rdfs = np.array([np.histogram(di,bins=bins)[0]*weights for di in d])

    return rdfs, bins
'''
#-----------------------------------------------------------------
def calc_bound_rdf(traj,nL,rbins,tbins,rrange,bound_intervals):

    def parfunc(bi):
        ri,ti = calc_LSL_distance(traj,nL,bi)
        return (ri,ti)

    n_proc = min(4,len(bound_intervals))
    out = Parallel(n_jobs=n_proc)(delayed(parfunc)(bi) for bi in bound_intervals)

    r = [xi[0] for xi in out]
    t = [xi[1] for xi in out]
...
'''
#-----------------------------------------------------------------
def calc_bound_rdf(trajfiles,nL,rbins,tbins,rrange,bound_intervals=None):
...
if __name__=="__main__":
    ...
    nC = 1
    traj_number = 0
    rundir = "./boxsize_5_5_5/run_bulk_nL{}_nC{}/trajectory_{}/".format(nLtag,nC,traj_number)

    # Set number of ligands vs inert crowders
    nL = nLtag

    #config_indices = range(0,50000,1000)
    trajfiles = [rundir+'LigandDiffusion_out_bulk.h5']

    # Build bins for radial distribution function of ligands as function of time
    traj = readdy.Trajectory(trajfiles[0])
    ...
    bound_intervals = [(10000,100000000)]
   
    hist2d, r_edges, t_edges = calc_bound_rdf(trajfiles,nL,rbins,tbins,rrange,bound_intervals)
    ...
    mean_rdf = hist2d[:,0]/np.sum(hist2d[:,0])
