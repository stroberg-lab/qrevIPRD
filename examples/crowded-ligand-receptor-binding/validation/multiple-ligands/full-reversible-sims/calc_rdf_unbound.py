...
def calc_AB_distance(traj,typeA,typeB,nA,nB,timerange=None):
    ...
    dt = timestep[1]-timestep[0]	# frequency of data recordings from simulation
    if timerange==None:
       ...
       if timestep[timeinds].size != 0: #not empty
           timerange = timeinds[0]
           tshifted = timerange-timerange[0]
       else:
           timerange = []
           tshifted = None
    ...
    for trajfile in trajfiles:
        ...
        try:
            ...
            def parfunc(bi):
                ...
                return (ri,ti)
            n_proc = 4
            out = Parallel(n_jobs=n_proc)(delayed(parfunc)(bi) for bi in unbound_intervals[0:nmax])
...
if __name__=="__main__":
    ...
    nmax = 1000
    hist2d, r_edges, t_edges = calc_unbound_rdf(trajfiles,nL,rbins,tbins,rrange,nmax)
    ...
    outfile = rundir + "unbound_rdf_time_2dhist.txt"
