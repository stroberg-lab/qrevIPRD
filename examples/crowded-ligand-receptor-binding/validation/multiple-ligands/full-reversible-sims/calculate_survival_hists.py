...
if __name__=="__main__":
    ...
    for ntraj in ntrajs:
        ...
        if savedata==True:
            ...
            if logrithmic_bins:
                outfilename = rundir + 'survival_hist_logrithmic_' + ntraj_tag + '.txt'
            else:
                outfilename = rundir + 'survival_hist_' + ntraj_tag + '.txt'
            outputdata = np.vstack((bin_lhs,bin_widths,survival_hist)).transpose()
            np.savetxt(outfilename,outputdata)

            if logrithmic_bins:
                outfilename_bound = rundir + 'bound_hist_logrithmic_' + ntraj_tag + '.txt'
            else:
                outfilename_bound = rundir + 'bound_hist_' + ntraj_tag + '.txt'
