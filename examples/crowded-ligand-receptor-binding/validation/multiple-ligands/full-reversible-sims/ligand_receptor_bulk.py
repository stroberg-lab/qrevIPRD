...
if __name__=="__main__":
    ...
    # Define particle sizes (approximate) and diffusion coefficients
    r_L = 1.0
    r_C = 1.0
    r_S = 1.0
    #r_complex = pow(pow(r_L+r_C,3.)+pow(r_R+r_C,3.),1./3.)-r_C		# conserves excluded volume
    r_SL = pow(pow(r_L,3.)+pow(r_S,3.),1./3.)			# conserves ligand-receptor volume

    kbT_over_sixPiEta = 1.0	# proportionality constant for stokes radius and diffusion coefficient
    D_L = kbT_over_sixPiEta/r_L
    D_C = kbT_over_sixPiEta/r_C
    D_S = kbT_over_sixPiEta/r_S
    D_SL = kbT_over_sixPiEta/r_SL

    # Define species types for simulation
    system.add_species("C", D_C)	# Crowding agent with diffusion constant D_C
    system.add_species("L", D_L)	# Ligand
    system.add_species("S", D_S)	# Receptor
    system.add_species("SL",D_SL)	# Ligand-receptor complex

    # Define reactions
    react_radius = r_L + r_S
    ...
    sigma_C = 2.*r_C		# effective diamteter for crowder
    sigma_L = 2.*r_L		# effective diameter for ligand
    sigma_S = 2.*r_S		# effective diameter for receptor
    sigma_SL = 2.*r_SL		# effective diameter for receptor-ligand complex
    ...
    set_cutshifted_lj("L","C",sigma_mix(sigma_L,sigma_C))
    set_cutshifted_lj("S","S",sigma_S)
    set_cutshifted_lj("S","C",sigma_mix(sigma_S,sigma_C))
    set_cutshifted_lj("S","SL",sigma_mix(sigma_S,sigma_SL))
    set_cutshifted_lj("C","C",sigma_C)
    set_cutshifted_lj("C","SL",sigma_mix(sigma_C,sigma_SL))
    set_cutshifted_lj("SL","SL",sigma_SL)
    ...
    trajectory.convert_to_xyz(particle_radii={'C': r_C,'L': r_L,'S': r_S,'SA': r_SL})
