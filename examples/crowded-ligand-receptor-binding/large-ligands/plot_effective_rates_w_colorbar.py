...
if __name__=="__main__":
    ...
    for i in range(len(kao)):
        for j in range(len(nCs)):
            tau_n[i][j] = tau_out[i*len(nCs) + j][0]
            n_mean[i][j] = tau_out[i*len(nCs) + j][1]
    ...
    #---------------------------------------------------------------------------------------------
    # Plot effective rate constants and residence time and long-time association rate
    #---------------------------------------------------------------------------------------------
    figK, axK = plt.subplots(1,2,figsize=(12,5))
    ...
    for i,kaoi in enumerate(kao):
        ...
        kons = [n_meanj/tau_nj for (n_meanj, tau_nj) in zip(n_mean[i],tau_n[i])]
        koffs = [(1.-n_meanj)/tau_nj for (n_meanj, tau_nj) in zip(n_mean[i],tau_n[i])]


        p = axK[0].plot(phi,kons/kons[0],'s-',color=colorVal,label=r"$k_{{a}}^{{o}}={}$".format(kaoi))
        axK[1].plot(phi,koffs/koffs[0],'s-',color=colorVal,label=r"$k_{{a}}^{{o}}={}$".format(kaoi))


    
    axK[0].set_xlabel(r'$\phi$',fontsize=18)
    axK[0].set_ylabel(r'$k_{\text{on}}^{\text{eff}}/k_{\text{on}}^{\text{eff,o}}$',fontsize=18)
    #axK[1][0].set_yscale('log')

    axK[1].set_xlabel(r'$\phi$',fontsize=18)
    axK[1].set_ylabel(r'$k_{\text{off}}^{\text{eff}}/k_{\text{off}}^\text{{eff,o}}$',fontsize=18)
    #axK[1][1].set_xscale('log')
    #axK[1][1].set_yscale('log')
    ...
    figK.subplots_adjust(right=0.8,bottom=0.2,wspace=0.45,hspace=0.4)
    cbar_ax = figK.add_axes([0.85, 0.2, 0.02, 0.65])	#[left, bottom, width, height]
    cbar = figK.colorbar(scalarMap, cax=cbar_ax)
    cbar.set_label(r'$k_{a}^{\text{o}}$',fontsize=18)

    figK.savefig("./Figures/effective_rates_w_colorbar.pdf")
