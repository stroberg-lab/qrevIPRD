...
#############################################################################################################
class LSCModel:
    """ Class for defining models with ligand, a sensor, and crowders (L,S,C) for simulation using qrev. """
    ...
    def __init__(self,nL,nC,nLtag=None):
        ...
        #self.r_SL = self.r_S			# receptor absorbs ligand volume

        self.kbT_over_sixPiEta = 0.5		# proportionality constant for stokes radius and diffusion coefficient
        ...
        self.dt = 1E-5
        bound_sim_sample_freq = int(100.*1e-4/self.dt) #100.
        ...
        self.sigma_mix = lambda sig1,sig2: (sig1+sig2)/2.	# Lorentz-Berthelot Rule for interaction distance
        ...
        self.weight_L = (self.r_L/self.r_S)**3.			# weight for placing complex between S and L reactants
        self.weight_S = 1. - self.weight_L	# weight for placing complex between S and L reactants

        # Set number of dissociation attempts for MC sampling
    ...
    def calc_dissociation_prob(self,trajfile,savefile):
        return calculate_unbinding_moves(trajfile, self.potdict, self.rneighbor, self.r_react, 
                                         self.weight_L, self.weight_S, savefile=savefile, tau_mol=self.tau_mol, n_samples=self.n_samples)
